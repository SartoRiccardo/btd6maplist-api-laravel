...
class MaplistTest extends TestCase
{
    ...
    {
        $format = FormatConstants::MAPLIST;
    ...
    public function test_three_completions_same_map_no_modifiers_counted_once(): void
    {
        ...
        $expectedPoints = $this->lbHelper->calcMlUserPoints($user->discord_id, FormatConstants::MAPLIST);
        $actualPoints = $this->lbHelper->getLbScore($user->discord_id, FormatConstants::MAPLIST, 'points');
    ...
    public function test_no_geraldo_modifier_applied(): void
    {
        ...
        CompletionMeta::factory()
            ...
            ->withPlayers([$user])
            ->accepted()
            ->create(['format_id' => FormatConstants::MAPLIST, 'no_geraldo' => true]);
    ...
    public function test_tied_scores_have_same_ranking(): void
    {
        ...
        $baseMetaFactory = CompletionMeta::factory()
            ->state(['format_id' => FormatConstants::MAPLIST])
            ->for($comp1)
            ->accepted()
            ->standard();
        ...
        $actual = $this->getJson('/api/formats/1/leaderboard')
            ->assertStatus(200)
            ->json();
    ...
    public function test_expert_list_completions_not_counted_for_maplist(): void
    {
        $user = User::factory()->create();
        $map = Map::factory()->withMeta(['placement_curver' => 1])->create();
    ...
    public function test_map_not_on_list_placement_gt_50(): void
    {
        $user = User::factory()->create();
        $mapOnList = Map::factory()->withMeta(['placement_curver' => 1])->create();
        $mapOffList = Map::factory()->withMeta(['placement_curver' => 51])->create();

        $comp1 = Completion::factory()->create(['map_code' => $mapOnList->code]);
        CompletionMeta::factory()
            ->for($comp1)
            ->withPlayers([$user])
            ->accepted()
        ...
        $comp2 = Completion::factory()->create(['map_code' => $mapOffList->code]);
        CompletionMeta::factory()
            ->for($comp2)
            ->withPlayers([$user])
            ->accepted()
    ...
    public function test_map_not_on_list_placement_null(): void
    {
        $user = User::factory()->create();
        $mapOnList = Map::factory()->withMeta(['placement_curver' => 1])->create();
        $mapOffList = Map::factory()->withMeta(['placement_curver' => null])->create();
    ...
    public function test_pagination_per_page_1(): void
    {
        ...
        $actual = $this->getJson('/api/formats/1/leaderboard?per_page=1')
            ->assertStatus(200)
            ->json();
